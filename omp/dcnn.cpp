...
void single_epoch(std::vector<matrix_t> &X, std::vector<matrix_t> &Y,
  ...
  std::vector<tensor_t> linearComp(BATCH_SIZE, init(num_layers, 1, 1, 0.0));
  std::vector<tensor_t> activationComp(BATCH_SIZE, init(num_layers, 1, 1, 0.0));
  std::vector<tensor_t> gradLinear(BATCH_SIZE, init(num_layers, 1, 1, 0.0));
  std::vector<tensor_t> gradActivation(BATCH_SIZE, init(num_layers, 1, 1, 0.0));
  std::vector<tensor_t> gradWeights(BATCH_SIZE, init(num_layers, 1, 1, 0.0));
  ...
  for (s = 0; s < X.size(); s += BATCH_SIZE) {
    ...
    for (b = 0; b < BATCH_SIZE; b++) {
      ...
      for (i = 0; i < num_layers; i++) {
        // linear
        if (i == 0)
          linearComp[b][i] = linearForward(X[s+b], weights[i]);
        else
          linearComp[b][i] = linearForward(activationComp[b][i-1], weights[i]);

        // activation
        switch (layer_types[i]) {
          case SIGM:
          activationComp[b][i] = sigmForward(linearComp[b][i]);
          break;
        case SOFT:
          activationComp[b][i] = softForward(linearComp[b][i]);
          break;
        case TANH:
          activationComp[b][i] = tanhForward(linearComp[b][i]);
          break;
        case RELU:
          activationComp[b][i] = reluForward(linearComp[b][i]);
          break;
        default:
          activationComp[b][i] = init(1, 1, 0.0);
          break;
        }
      }
      ...
      for (i = num_layers - 1; i >= 0; i--) {
        ...
        switch (layer_types[i]) {
        case SIGM:
          gradLinear[b][i] = sigmBackward(linearComp[b][i], activationComp[b][i],
                                     gradActivation[b][i]);
          break;
        case SOFT:
          gradLinear[b][i] = softBackward(Y[s+b], linearComp[b][i],
                                     activationComp[b][i], gradActivation[b][i]);
          break;
        case TANH:
          gradLinear[b][i] = tanhBackward(linearComp[b][i], activationComp[b][i],
                                     gradActivation[b][i]);
          break;
        case RELU:
          gradLinear[b][i] = reluBackward(linearComp[b][i], activationComp[b][i],
                                     gradActivation[b][i]);
          break;
        default:
          gradLinear[b][i] = init(1, 1, 0.0);
          break;
        }

        if (i == 0) {
          gradWeights[b][i] = linearBackward1(X[s+b], gradLinear[b][i]);
        } else {
          gradWeights[b][i] =
              linearBackward1(activationComp[b][i - 1], gradLinear[b][i]);
          gradActivation[b][i - 1] = linearBackward2(weights[i], gradLinear[b][i]);
        }
      }
    }
    ...
    for (i = 0; i < num_layers; i++) {
      for (b = 0; b < BATCH_SIZE; b++) {
        weights[i] =
            subtract(weights[i], multiply(gradWeights[b][i], learning_rate));
      }
    }
  }
