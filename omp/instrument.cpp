...
static char *activity_name[ACTIVITY_COUNT] = {"unknown", "setup", "train", "comm", "predict"};
...
void show_activity(FILE *f) {
  ...
  for (a = 0; a < ACTIVITY_COUNT; a++) {
    ...
    fprintf(f, "    %8d secs    %5.1f %%    %s\n", (int) secs, pct, activity_name[a]);
  }
  fprintf(f, "    %8d secs    %5.1f %%    elapsed\n", (int) (elapsed), 100.0);
}
